...
@section('title', 'გაუქმების მიზეზი')
              ...
              <div class="table-responsive">
                <table class="table">
                    ...
                    @foreach($reasons as $index => $reason)
                        <tr>
                            <td>{{$index + 1}}</td>
                            <td>{{$reason->type == "PROVIDER" ? "მძღოლი" : "მომხმარებელი"}}</td>
                            <td>{{$reason->reason}}</td>
                            <td>
                                @if($reason->status==1)
                            ...
                            <td>
                                <form action="{{ route('admin.reason.destroy', $reason->id) }}" method="POST">
                                    ...
                                    <a href="{{ route('admin.reason.edit', $reason->id) }}" class="btn btn-info"><i class="fa fa-pencil"></i> ცვლილება</a>
                                    @endcan
