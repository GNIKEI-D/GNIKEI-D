                ...
                <div class="form-group">
                    ...
                    <div class="col-xs-5">
                        <select class="form-control" name="status" required id="status">
                            <option @if($lostitem->status == 'open') required='required' @endif value="open">???????</option>
                            <option @if($lostitem->status == 'closed') required='required' @endif value="closed">???????????</option>
                        </select>
                    </div>
                </div>
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                if (data.data.length > 0) {
                    ...
                    requestList.html(`<tr><td colspan="4">???????? ??? ????????</td></tr>`);
                }
            }
        });
